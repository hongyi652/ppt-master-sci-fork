...
def generate_markdown(results: list[ImageAnalysis], canvas_key: str) -> None:
    ...
    print("> Image display integrity is the default. Use `Display mode = complete`")
    print("> for source/document images and preserve them with `meet`; switch to")
    print("> `crop-ok` only for decorative backgrounds where edge loss is harmless.")
    ...
    if has_layout:
        print("| Filename | Size | Ratio | Display mode | Category | Img Area (SxS) | Text Area (SxS) | Intent | Usage | Type | Status | Generation Description |")
        print("|----------|------|-------|--------------|----------|----------------|-----------------|--------|-------|------|--------|-----------------------|")
    else:
        print("| Filename | Size | Ratio | Display mode | Category | Intent | Usage | Type | Status | Generation Description |")
        print("|----------|------|-------|--------------|----------|--------|-------|------|--------|-----------------------|")
    ...
    for img in results:
        ...
        if has_layout:
            ...
            print(f"| {img['filename']} | {img['width']}x{img['height']} | {ratio_str} | complete | {img['layout_hint']} | {img_area} | {text_area} | (to be filled) | {img.get('usage_count', 1)} refs | {image_type} | {status} | - |")
        else:
            print(f"| {img['filename']} | {img['width']}x{img['height']} | {ratio_str} | complete | {img['layout_hint']} | (to be filled) | {img.get('usage_count', 1)} refs | {image_type} | {status} | - |")
...
def save_csv(results: list[ImageAnalysis], csv_path: str) -> None:
    ...
    with open(csv_path, 'w', encoding='utf-8') as f:
        if has_layout:
            f.write("No,Filename,Width,Height,AspectRatio,PixelAspectRatio,RatioSource,UsageCount,DisplayRatioVariants,DisplayMode,AssetKind,SvgRenderable,PptxNativeSupported,SizeKB,Category,ImageArea_SxS,TextArea_SxS\n")
            for i, img in enumerate(results, 1):
                f.write(f"{i},{img['filename']},{img['width']},{img['height']},{img['aspect_ratio']:.2f},{img.get('pixel_aspect_ratio', img['aspect_ratio']):.2f},{img.get('ratio_source', 'pixel')},{img.get('usage_count', 1)},{img.get('display_ratio_variants', '')},complete,{img.get('asset_kind', 'bitmap')},{img.get('svg_renderable', True)},{img.get('pptx_native_supported', True)},{img['filesize_kb']:.1f},{img['layout_hint']},{img['image_w']}x{img['image_h']},{img['text_w']}x{img['text_h']}\n")
        else:
            f.write("No,Filename,Width,Height,AspectRatio,PixelAspectRatio,RatioSource,UsageCount,DisplayRatioVariants,DisplayMode,AssetKind,SvgRenderable,PptxNativeSupported,SizeKB,Category\n")
            for i, img in enumerate(results, 1):
