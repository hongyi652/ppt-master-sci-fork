...
(function () {
    ...
    var MESSAGES = {
        en: {
            ...
            btn_open_ppt_folder: "Open PPT folder",
            ...
            err_open_ppt_folder: "Failed to open PPT folder: ",
            ok_open_ppt_folder: "Opened PPT folder",
            ...
            nav_empty: "— / —",
            progress_stage_initializing: "Initializing project",
            progress_stage_preparing_sources: "Preparing sources",
            progress_stage_planning_deck: "Planning deck",
            progress_stage_acquiring_assets: "Preparing assets",
            progress_stage_generating_slides: "Generating slides",
            progress_stage_generating_notes: "Generating speaker notes",
            progress_stage_finalizing_svg: "Finalizing SVGs",
            progress_stage_exporting_pptx: "Exporting PPTX",
            progress_stage_completed: "Export complete",
            progress_unknown_stage: "Working...",
            progress_slides: "Slides",
            progress_finalized: "Finalized",
            progress_notes: "Notes",
            progress_exports: "Exports",
            progress_latest_export: "Latest export",
        ...
        zh: {
            ...
            err_save: "保存失败:",
            err_open_ppt_folder: "打开PPT文件夹失败:",
            ok_open_ppt_folder: "已打开PPT文件夹",
            ...
            nav_counter: "{current} / {total}",
            nav_empty: "— / —",
            progress_stage_initializing: "初始化项目",
            progress_stage_preparing_sources: "准备源材料",
            progress_stage_planning_deck: "规划PPT内容",
            progress_stage_acquiring_assets: "准备图片与公式素材",
            progress_stage_generating_slides: "正在生成页面",
            progress_stage_generating_notes: "正在生成备注",
            progress_stage_finalizing_svg: "正在整理导出SVG",
            progress_stage_exporting_pptx: "正在导出PPTX",
            progress_stage_completed: "导出完成",
            progress_unknown_stage: "处理中……",
            progress_slides: "页面",
            progress_finalized: "已整理",
            progress_notes: "备注",
            progress_exports: "导出",
            progress_latest_export: "最新导出",
            progress_status_ready: "已完成",
            progress_status_pending: "待完成",
            progress_unknown_total: "?"
        }
    };
    ...
    function applyI18n() {
        ...
        updateNavLabel();
        updateProgressPanel(lastProgress);
    }

    function updatePageTitle() {
        var title = t("page_title");
        if (projectName) {
            title += " · " + projectName;
        }
        ...
        if (projectNameEl) {
            ...
            projectNameEl.title = projectPath || projectName || "";
        }
    }
    ...
    var progressDetailsEl = document.getElementById("progress-details");
    var btnOpenExportFolder = document.getElementById("btn-open-export-folder");
    ...
    function apiUrl(path) {
        apiSeq += 1;
        return path + (path.indexOf("?") >= 0 ? "&" : "?") + "_=" + Date.now() + "_" + apiSeq;
    }
    ...
    function progressStageLabel(stage) {
        var key = "progress_stage_" + (stage || "");
        var translated = t(key);
        return translated === key ? t("progress_unknown_stage") : translated;
    }

    function updateProgressPanel(data) {
        lastProgress = data || null;
        if (!progressStageEl || !progressPercentEl || !progressBarFillEl || !progressDetailsEl) return;

        var percent = Number(data && data.percent);
        if (!isFinite(percent)) percent = 0;
        percent = Math.max(0, Math.min(100, Math.round(percent)));

        var totalPages = Number(data && data.total_pages) || 0;
        var slidesGenerated = Number(data && data.slides_generated) || 0;
        var slidesFinalized = Number(data && data.slides_finalized) || 0;
        var notesReady = !!(data && data.notes_ready);
        var exportsCount = Number(data && data.exports_count) || 0;
        var latestExportName = data && data.latest_export_name ? String(data.latest_export_name) : "";
        var totalLabel = totalPages > 0 ? String(totalPages) : t("progress_unknown_total");

        progressStageEl.textContent = progressStageLabel(data && data.stage);
        progressPercentEl.textContent = percent + "%";
        progressBarFillEl.style.width = percent + "%";

        var lines = [
            t("progress_slides") + ": " + slidesGenerated + " / " + totalLabel,
            t("progress_finalized") + ": " + slidesFinalized + " / " + totalLabel,
            t("progress_notes") + ": " + t(notesReady ? "progress_status_ready" : "progress_status_pending"),
            t("progress_exports") + ": " + exportsCount
        ];
        if (latestExportName) {
            lines.push(t("progress_latest_export") + ": " + latestExportName);
        }

        progressDetailsEl.innerHTML = lines.map(function (line) {
            return '<div class="progress-detail-line">' + escapeHtml(line) + '</div>';
        }).join("");
    }

    function loadProgress() {
        return fetch(apiUrl("/api/progress"), { cache: "no-store" })
            .then(jsonOrThrow)
            .then(function (data) {
    ...
    // ================================================================
    function loadSlides() {
        return fetch(apiUrl("/api/slides"), { cache: "no-store" })
            .then(function (res) { return res.json(); })
            .then(function (data) {
                var incomingProjectPath = data.project_path || "";
                var projectChanged = !!incomingProjectPath && incomingProjectPath !== projectPath;
                if (projectChanged) {
                    projectName = data.project_name || "";
                    projectPath = incomingProjectPath;
                    currentSlide = null;
                    slideNames = [];
                    slideMtimes = {};
                    slideAnnotations = {};
                ...
                if (!currentSlide || !currentExists) {
                    ...
                    var hasPendingEdits = annotationText.value.trim().length > 0 || selectedElementIds.size > 0;
                    if (hasPendingEdits) {
                        showReloadBanner(currentSlide);
                    } else {
                        var currentItem = slideListEl.querySelector('.slide-item[data-name="' + cssAttr(currentSlide) + '"]');
                        selectSlide(currentSlide, currentItem || undefined);
                    }
                }
                updateNavLabel();
            })
    ...
    function selectSlide(name, el) {
        ...
        fetch(apiUrl("/api/slide/" + encodeURIComponent(name)), { cache: "no-store" })
            .then(function (res) { return res.json(); })
            .then(function (data) {
                if (data.error) {
    ...
    if (btnOpenExportFolder) {
        btnOpenExportFolder.addEventListener("click", function () {
            fetch(apiUrl("/api/open-export-folder"), { method: "POST" })
                .then(jsonOrThrow)
                .then(function (data) {
                    var target = data.selected_file || data.exports_dir || "";
                    showSuccess(t("ok_open_ppt_folder") + (target ? ": " + target : ""));
                })
                .catch(function (err) {
                    console.error("openExportFolder:", err);
                    showError(t("err_open_ppt_folder") + err.message);
                });
        });
    }
    ...
    function showSuccess(msg) {
        var banner = document.createElement("div");
        banner.style.cssText = "position:fixed;top:38px;left:0;right:0;padding:8px 16px;background:#10b981;color:#ffffff;font-size:12px;text-align:center;z-index:998;cursor:pointer;";
        banner.textContent = msg;
        banner.onclick = function () { banner.remove(); };
        document.body.appendChild(banner);
    ...
    function loadConfig() {
        return fetch(apiUrl("/api/config"), { cache: "no-store" })
            .then(function (res) { return res.json(); })
            .then(function (data) {
                liveMode = !!data.live;
                projectName = data.project_name || "";
