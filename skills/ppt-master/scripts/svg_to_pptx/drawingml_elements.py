...
def _get_inline_attr(elem: ET.Element, attr: str) -> str | None:
    """Read a run-level attribute from the element or its inline style."""
    val = elem.get(attr)
    if val is not None:
        return val
...
def _parse_inline_font_size(raw: str | None, inherited_px: float, scale_y: float) -> float | None:
    """Parse inline SVG font-size values, including relative units like em/%."""
    if raw is None:
        return None
    ...
    if value.endswith('em'):
        amount = _parse_number(value[:-2])
        return inherited_px * amount if amount is not None else None
    if value.endswith('%'):
        amount = _parse_number(value[:-1])
        return inherited_px * amount / 100.0 if amount is not None else None
    if value.endswith('px'):
        amount = _parse_number(value[:-2])
        return amount * scale_y if amount is not None else None
    if value.endswith('pt'):
        amount = _parse_number(value[:-2])
        return amount * (96.0 / 72.0) * scale_y if amount is not None else None

    amount = _parse_number(value)
    return amount * scale_y if amount is not None else None


def _parse_baseline_shift(raw: str | None) -> int | None:
    """Map SVG baseline-shift values onto DrawingML baseline offsets."""
    if raw is None:
        return None

    value = raw.strip().lower()
    if not value or value in {'baseline', 'normal', 'initial', 'inherit', 'unset'}:
        return None
    if value in {'super', 'sup', 'superscript'}:
        return 30000
    if value in {'sub', 'subscript'}:
        return -25000

    try:
        numeric = float(value.rstrip('%abcdefghijklmnopqrstuvwxyz'))
    except ValueError:
        return None
    if numeric > 0:
        return 30000
...
def _override_run_attrs(
    ...
    font_weight = _get_inline_attr(tspan, 'font-weight')
    if font_weight:
        run_attrs['font_weight'] = font_weight
    child_fill = _get_inline_attr(tspan, 'fill')
    if child_fill:
        run_attrs['fill_raw'] = child_fill
        c = parse_hex_color(child_fill)
        if c:
            run_attrs['fill'] = c
    stroke = _get_inline_attr(tspan, 'stroke')
    if stroke:
        run_attrs['stroke_raw'] = stroke
    stroke_width = _get_inline_attr(tspan, 'stroke-width')
    if stroke_width:
        run_attrs['stroke_width'] = _f(stroke_width, run_attrs.get('stroke_width', 1.0))
    stroke_opacity = _get_inline_attr(tspan, 'stroke-opacity')
    if stroke_opacity:
        try:
            run_attrs['stroke_opacity'] = float(stroke_opacity)
        except ValueError:
            pass
    font_size = _get_inline_attr(tspan, 'font-size')
    if font_size:
        parsed_font_size = _parse_inline_font_size(
            font_size,
            inherited_px=run_attrs['font_size'],
            scale_y=run_attrs.get('font_scale_y', 1.0),
        )
        if parsed_font_size is not None:
            run_attrs['font_size'] = parsed_font_size
    font_family = _get_inline_attr(tspan, 'font-family')
    if font_family:
        run_attrs['font_family'] = font_family
    font_style = _get_inline_attr(tspan, 'font-style')
    if font_style:
        run_attrs['font_style'] = font_style
    text_decoration = _get_inline_attr(tspan, 'text-decoration')
    if text_decoration:
        run_attrs['text_decoration'] = text_decoration

    baseline_shift = _get_inline_attr(tspan, 'baseline-shift')
    if baseline_shift is not None:
        baseline = _parse_baseline_shift(baseline_shift)
        if baseline is None:
            run_attrs.pop('baseline', None)
        else:
            run_attrs['baseline'] = baseline
    return run_attrs
...
def _build_run_xml(
    ...
    baseline_attr = f' baseline="{int(baseline)}"' if baseline else ''
    ...
    return f'''<a:r>
<a:rPr lang="zh-CN" sz="{sz}"{b_attr}{i_attr}{u_attr}{strike_attr}{baseline_attr} dirty="0">
{outline_xml}
{fill_xml}
{effect_xml}
...
_SVG_LENGTH_RE = re.compile(rb'([+-]?(?:\d+(?:\.\d*)?|\.\d+))')


def _parse_svg_length(value: str | None) -> float | None:
    """Parse an SVG width/height length value, ignoring units for ratio math."""
    if not value:
        return None
    match = _SVG_LENGTH_RE.search(value.encode('utf-8', errors='ignore'))
    if not match:
        return None
    try:
...
def _read_svg_size(data: bytes) -> tuple[float | None, float | None]:
    """Read intrinsic SVG dimensions from width/height or viewBox."""
    try:
    ...
    view_parts = view_box.replace(',', ' ').split()
    vb_w = None
    vb_h = None
    if len(view_parts) == 4:
        try:
            vb_w = float(view_parts[2])
            vb_h = float(view_parts[3])
        except ValueError:
            vb_w = None
            vb_h = None
    ...
    if width is not None and height is not None and width > 0 and height > 0:
        return (width, height)
    if width is not None and width > 0 and vb_w and vb_h and vb_w > 0 and vb_h > 0:
        return (width, width * vb_h / vb_w)
    if height is not None and height > 0 and vb_w and vb_h and vb_w > 0 and vb_h > 0:
...
def _read_image_size(data: bytes) -> tuple[float | None, float | None]:
    ...
    legacy stretch behaviour. SVG inputs are parsed from ``width`` / ``height``
    or ``viewBox`` because Pillow cannot read intrinsic SVG dimensions.
    """
    stripped = data.lstrip()
    if stripped.startswith(b'<?xml') or stripped.startswith(b'<svg'):
        svg_w, svg_h = _read_svg_size(data)
