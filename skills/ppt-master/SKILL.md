...
> Short / inline formulas marked `inline-or-callout` or `formula-compact` must remain near text/callout scale; do not enlarge them into hero-size visuals unless the page outline explicitly makes the formula the main object. But the same table also defines the readable-size floor: do **not** place a formula SVG below its `Recommended display`, even on dense pages.
>
