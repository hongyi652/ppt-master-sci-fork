...
> **Completeness-first rule**: patterns that crop, mask, or full-bleed an image are valid only for rows marked `Display mode: crop-ok`. Source-document images, charts, screenshots, product photos, and evidence images default to complete display with `preserveAspectRatio="xMidYMid meet"`.
