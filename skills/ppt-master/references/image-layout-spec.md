...
Layout rules for pages where source images must remain visually complete. Strategist and Executor use these rules to choose image placement from native dimensions before composing text around the image.
...
**Global override**: apply the same completeness principle to every content-bearing source image, not only side-by-side pages.

> **Scope**: the ratio table is mandatory for content-bearing source images and side-by-side intent. Hero / full-bleed / atmosphere pages may crop only when the row explicitly marks `Display mode: crop-ok`; otherwise preserve the full image and adapt the composition.

---

## 0. Completeness-First Default
...
|---|---|---|
| Data chart / screenshot / paper figure / diagram | `complete` | Match the container to native ratio; use `meet` |
| Product photo / person photo / architecture / evidence image | `complete` | Choose top-bottom or left-right from image ratio; use `meet` |
| Multi-image comparison | `complete` | Use same-size cells, but each image uses `meet` |
| Decorative background / atmosphere image | `crop-ok` only when declared | Full-bleed or dominant-zone `slice` is allowed |

## Layout Decision Flow

Start by reading the source image dimensions. Do not choose a fixed frame before width, height, and ratio are known.

```
1. Decide narrative intent (hero / atmosphere / side-by-side / accent) — see strategist.md §h
2. Set `Display mode`: `complete` by default; `crop-ok` only for decorative/full-bleed backgrounds.
3. Decide narrative intent (hero / atmosphere / side-by-side / accent) — see strategist.md §h.
4. Get image original dimensions → Calculate ratio (width/height).
5. Select layout type based on ratio and canvas orientation.
6. Calculate maximum complete-display size for the image.
7. Allocate remaining space for text area.
8. Fill results into the Design Specification's image resource list.
```
...
| `Display mode` | `complete` by default; `crop-ok` only for decorative/full-bleed background rows |
