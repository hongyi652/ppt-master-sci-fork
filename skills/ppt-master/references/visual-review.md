...
| H8 | Image rendering broken | `<image>` empty / broken-image / severe distortion / content-bearing image cropped | fix `href`, adjust `preserveAspectRatio`, set `Display mode: complete` / `meet` if face/data is cropped |
