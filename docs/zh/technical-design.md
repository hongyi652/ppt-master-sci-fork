...
用户输入 (PDF/DOCX/XLSX/URL/Markdown)
    ↓
[源内容转换] → source_to_md/mineru_to_md.py / doc_to_md.py / excel_to_md.py / ppt_to_md.py / web_to_md.py
    ↓
...
[模板处理（可选）] — 默认跳过，直接自由设计
    ...
    ↓
[Strategist] 策略师 - 设计规范；八项确认仅在分步模式阻塞 → design_spec.md + spec_lock.md
    ↓
[Image Acquisition] 图片获取（当资源列表中有需要 AI 生成或网络搜索的图片时）
    ↓
[Executor] 执行师
    ├── 视觉构建：连续生成所有 SVG 页面 → svg_output/
    ├── 质量检查前草稿导出 → exports/pre_qc_draft.pptx
...
输出：
    exports/
    ├── pre_qc_draft.pptx                      ← 质量检查前的无讲稿 PPTX 草稿快照
...
项目布局里非显然的一点是 `import-sources` 的**源文件保护分层**：用户提供的原始文档（`.pdf`、`.docx`、`.pptx`、`.xlsx` 等）始终 *copy* 到 `sources/`，即使传入 `--move` 也不会移动原件。生成的 Markdown、伴随 `_files/` 目录等中间产物，才会在操作者要求清理时移动；仓库内的临时文件也默认移动，除非显式传入 `--copy`。这套分层对应自然的风险画像——用户资产必须保护，生成产物可以清理。
...
**确认 gate 随执行模式变化。** 分步模式仍把八项打包确认（画布 / 页数 / 受众 / 风格 / 配色 / 图标 / 排版 / 图像）作为单一阻塞决策点，并且先询问 PPT 正文语言。一键模式跳过这两次询问：Strategist 自动锁定推荐语言和八项方案，写入 `design_spec.md` / `spec_lock.md`，随后流水线不中断地跑到结束。八项仍然打包处理的理由是：设计选项之间相关（配色影响图标库、影响排版），一起决能产出一致的决策；分散到各阶段确认会引入互相矛盾的用户输入，最后被迫回退重做。
