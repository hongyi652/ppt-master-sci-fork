...
项目布局里非显然的一点是 `import-sources` 的**只读源输入**：用户提供的文档、图片、Markdown、数据文件、源文件夹，以及通配符展开得到的文件夹内容，都会始终 *copy* 到 `sources/`。`--move` 只为兼容旧命令而保留，实际会被忽略。伴随 `_files/` 目录也只复制，因此导入过程不会清空任何源文件夹。清理是单独的、必须由用户明确要求的动作，不属于源输入流程。
