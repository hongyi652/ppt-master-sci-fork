...
| 层级 | 默认 | 原因 |
|---|---|---|
| 页间转场 | `none` | 默认不添加页间切换效果 |
| 页内元素动画 | `none` | 默认不添加对象入场动画；需要演示节奏时再显式开启 |

修改设置只需对同一份 `svg_output/`（或 `svg_final/`）重跑 `svg_to_pptx.py`，无需重新跑 LLM。开启页内动画时传 `-a fade`、`-a auto` 或其他效果名；保持关闭时传 `-a none` 或省略该参数。

## 对象级自定义动画

对象级动画是可选导出策略。若需要更具体的演示节奏，例如标题先淡入、图表第二个出现、关键注释最后飞入，可以使用可选的 `animations.json` sidecar。SVG 仍然只保存静态视觉结构；sidecar 只控制 PPTX 导出动画。
...
- `-t/--transition` — 效果名，或 `none` 禁用。默认 `none`。
- `--transition-duration` — 秒数，默认 `0.4`。
- `--auto-advance` — 秒数；不写则由演示者手动翻页。

## 页内元素动画

默认关闭。通过 `-a/--animation` 显式开启后，共有三种 Start 模式，**与 PowerPoint 动画窗格的 Start 下拉菜单一一对应**：
...
# 开启单一效果（默认走 after-previous 自动级联）
python3 skills/ppt-master/scripts/svg_to_pptx.py <project> -a fade
...
- `-a/--animation` — 效果名、`auto`、`mixed`、`random` 或 `none`。默认 `none`。
